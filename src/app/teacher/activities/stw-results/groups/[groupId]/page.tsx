...
export function generateStaticParams() {
  return activityGroups.map((group) => ({ groupId: group.id }));
}
...
export default async function GroupSubmissionPage({
  ...
  const group = activityGroups.find((item) => item.id === groupId);
  ...
  const presentMembers = groupMembers.filter(
    (student) => student && activityAttendance.find((item) => item.activityId === activityId && item.studentId === student.id)?.status !== "absent"
  );
  const absentMembers = groupMembers.filter(
    (student) => student && activityAttendance.find((item) => item.activityId === activityId && item.studentId === student.id)?.status === "absent"
  );
  const agreedMembers = presentMembers.filter((student) =>
    groupSubmissionAgreements.some((agreement) => agreement.activityId === activityId && agreement.groupId === group.id && agreement.studentId === student?.id && agreement.agreed)
  );
  const cards = groupThinkingCards.filter((card) => card.activityId === activityId && card.groupId === group.id);
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description="사진 속 도시의 변화 관찰하기 · 모둠원이 함께 작성한 결과입니다."
        action={
          <Link
            href="/teacher/activities/stw-results"
