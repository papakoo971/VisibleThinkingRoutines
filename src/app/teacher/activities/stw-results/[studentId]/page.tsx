...
import { students, thinkingCards } from "@/lib/mock-data";
...
export default async function StudentSubmissionPage({
  ...
  const studentCards = thinkingCards.filter((card) => card.studentId === student.id);
  ...
  return (
    <AppShell>
      <PageHeader
        ...
        description={`${student.className} · ${student.studentNumber} · 사진 속 도시의 변화 관찰하기`}
        action={
          <Link
