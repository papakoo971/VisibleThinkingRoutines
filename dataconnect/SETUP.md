...
Teacher result reads use `GetTeacherActivityResults`, which filters by the authenticated owner's UID before returning student cards and submission states. `SetActivityStatus` applies the same owner check before closing or reopening an activity; closed activities remain readable to assigned students but all student work mutations reject writes.

`UpdateThinkingCardTags` verifies the card's activity owner before saving default or custom tags and the `tagsPublic` flag. Student work queries return tag values only when the teacher has enabled public visibility.

Activities persist `instructions`, `materialUrl`, and `materialName`. The creation UI uploads supported files to `teachers/{teacherUid}/activities/{activityId}` and generates a QR PNG locally. The default bucket is `visible-thinking-routines.firebasestorage.app`; deployed rules limit creates and updates to the owning teacher path, supported MIME types, and files below 10MB, while authenticated students can read assigned material URLs.
